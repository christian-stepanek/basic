	...
	do
		...
		then
			...
			do
				...
				str="$(cdo -intlevel,10,100,1000,4000 ${origdir}/fesom/${var}.fesom.${i}${m}.nc /dev/null/test.nc 2>&1 > /dev/null)"
			...
			#test whether fesom output contains vertical level information that is necessary to interpolate the data to specific depths
			str="$(cdo -intlevel,10,100,1000,4000 ${origdir}/fesom/${var}.fesom.${i}.nc /dev/null/test.nc 2>&1 > /dev/null)"
			if [[ "$str" == *"intlevel (Abort): No processable variable found"* ]]
			then
				echo "adding vertical level information based on standard levels in AWI-ESM2 (check that this is correct\!\!\!)"
				#add nz1 information if necessary
				...
				#perform interpolation on original data if possible
				cdo -intlevel,10,100,1000,4000 ${origdir}/fesom/${var}.fesom.${i}.nc ${tmpdir}/${var}.fesom.${i}.int.nc #& parallel execution deactivated for testing
			fi
		fi
	done
done
...
printf "#################################################\n"
cdo -f nc select,code=129,130,133,138,152,155,year=${first_year}/${last_year} ${origdir}/echam/*_??????.01_echam ${tmpdir}/data_for_afterburner.nc
...
printf "#################################################\n"
cdo -f nc -t echam6 select,name=aps,year=${first_year}/${last_year} ${origdir}/echam/*_??????.01_echam ${tmpdir}/aps_${tmpstr}.nc
for var in u v z
do
...
do
	...
	do
		cdo cat ${origdir}/fesom/${var}.fesom.${i}.nc ${tmpdir}/${var}_${tmpstr}.nc &
	done
	for var in temp salt;
	do
...
cdo chname,MLD1,mlotst -divc,-1 ${tmpdir}/MLD1_${tmpstr}_remap.nc  ${tmpdir}/mlotst_${tmpstr}.nc &
wait
